...
/* The arcade is not generated — each machine is a folder written by hand — but
   its pages still have to be findable, so they are read off the disk rather
   than listed here. A machine nobody has linked yet is a machine Google will
   never see. */
const arcadePages = existsSync(join(ROOT, "arcade"))
  ? ["/arcade/"].concat(
      readdirSync(join(ROOT, "arcade"), { withFileTypes: true })
        .filter((entry) => entry.isDirectory() && existsSync(join(ROOT, "arcade", entry.name, "index.html")))
        .map((entry) => `/arcade/${entry.name}/`)
        .sort()
    )
  : [];

const sitemap = `<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by scripts/build-work-pages.mjs: the homepage, one URL per case
     study, and the arcade pages found on disk. Since 2026-09 the case studies
     are real pages, not only modals of "/". -->
